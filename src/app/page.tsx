  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <main className="flex-1 pt-14 pb-16 relative">
        ...
        {showWelcome && (
          <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/90 to-slate-950/60 z-10 flex items-center justify-center p-6">
            <div className="max-w-md w-full space-y-6 animate-in fade-in slide-in-from-bottom-4">
              {/* Logo */}
              ...
              <div className="grid grid-cols-2 gap-3">
                <Link
                  href="/stations"
                  className="flex flex-col items-center gap-2 bg-gradient-to-br from-blue-600/20 to-blue-800/10 border border-blue-500/20 rounded-2xl p-4 hover:border-blue-400/40 transition-all group"
                >
                ...
                <Link
                  href="/report"
                  className="flex flex-col items-center gap-2 bg-gradient-to-br from-red-600/20 to-red-800/10 border border-red-500/20 rounded-2xl p-4 hover:border-red-400/40 transition-all group"
                >
                ...
                <Link
                  href="/chat"
                  className="flex flex-col items-center gap-2 bg-gradient-to-br from-purple-600/20 to-purple-800/10 border border-purple-500/20 rounded-2xl p-4 hover:border-purple-400/40 transition-all group"
                >
                ...
                <button
                  ...
                  className="flex flex-col items-center gap-2 bg-gradient-to-br from-emerald-600/20 to-emerald-800/10 border border-emerald-500/20 rounded-2xl p-4 hover:border-emerald-400/40 transition-all group"
                >
        ...
        {/* Floating Action Buttons (when map visible) */}
        {!showWelcome && (
          <div className="absolute bottom-6 right-4 flex flex-col gap-3">
            <button
              ...
              title={isListening ? 'หยุดฟัง' : 'สั่งด้วยเสียง'}
            >
        ...
        {/* Voice Transcript */}
        {!showWelcome && isListening && transcript && (
          <div className="absolute top-4 left-4 right-4 bg-slate-800/90 backdrop-blur-sm rounded-xl p-4 border border-orange-500/30">
        ...
        {/* Login hint */}
        {!showWelcome && !user && (
          <div className="absolute top-4 left-4 right-4 bg-slate-800/90 backdrop-blur-sm rounded-xl p-3 border border-slate-600">
            <p className="text-xs text-slate-300">
              <Link href="/login" className="text-orange-400 font-medium hover:underline">
                เข้าสู่ระบบ
              </Link>{' '}
              เพื่อแจ้งเหตุและรายงาน
            </p>
          </div>
        )}
      </main>
